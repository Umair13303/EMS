...
app.controller('CreateUpdate_HREmployee_UIController', function ($scope, $http, Uttility, $filter) {
    ...
    $scope.Initialize = function () {
        ...
        $scope.Populate_HREmployee_List();
    };

    $scope.$watch('DataTableSetting.Filter', function (newValue) {
        if (newValue) {
            $scope.DataTableSetting.FilteredEmployeeList = $filter('filter')($scope.DataTableSetting.HREmployeeList, {
    ...
    $scope.Populate_HREmployee_List = function () {
        Uttility.startLoading();
        $http.get('/api/HumanResource/HREmployeeManagment/GET_HREmployeeList')
            .then(function (response) {
                $scope.DataTableSetting.HREmployeeList = response.data;
    ...
    $scope.Load_HREmployee_InfoById = function (item, IsEditMode) {
        ...
        $scope.HREmployee.EmploymentTypeId = item.EmploymentTypeId;
        $scope.HREmployee.JoiningDate = $filter('date')(item.DateofJoining, 'yyyy-MM-dd');
    ...
    $scope.Delete_HREmployee_ById = function (GuID) {
        Uttility.startLoading();
        $http.post('/api/HumanResource/HREmployeeManagment/Delete_RecordByGuID?GuID=' + GuID)
            .then(function (response) {
                Uttility.GetMessageBox(response.data.message, response.data.status);
                $scope.Initialize();
            })
