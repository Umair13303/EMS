...
namespace office360.Areas.Account.Controllers
{
    public class PCRExpenseManagmentController : ApiController
    {
        ...
        public IHttpActionResult UpsetDataIntoDB([FromUri] string OperationType, [FromBody] PCRExpense postedData)
        {
            try
            {
                ...
                else
                {
                    ...
                    int? OperationStatus = Is_Exist_PCRExpense(OperationType, postedData.GuID, postedData.Description, postedData.InvoiceNumber, postedData.NetAmount);
                    #endregion
                        ...
                        if (OperationStatus == (int?)Http_DB_Response.CODE_AUTHORIZED)
                        {
                            using (EMSIntegCubeEntities dal = new EMSIntegCubeEntities())
                            {
                                ...
                                else
                                {
                                    ...
                                    ExistingPCRExpense.UpdatedBy = 1;
                                    dal.SaveChanges();
        ...
        public IHttpActionResult GET_ExpenseCategoryList()
        {
            using (EMSIntegCubeEntities dal = new EMSIntegCubeEntities())
            {
                var ExpenseCategoryList = dal.ExpenseCategory.AsNoTracking().Where(x => x.Status == true)
                     .Select(x => new
                     {
                         x.Id,
                         x.Description,
        ...
        public IHttpActionResult GET_PCRExpenseList()
        {
            try
            {
                using (EMSIntegCubeEntities dal = new EMSIntegCubeEntities())
                {
                    ...
                    var PCRExpenseList = dal.PCRExpense.AsNoTracking().Where(x => x.CompanyId == CompanyId && x.Status == true).OrderByDescending(x => x.TransactionDate)
                        .Select(x => new
                        {
                            x.GuID,
                            x.TransactionDate,
